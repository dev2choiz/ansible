return {
  ...
  },
  {
    ...
    config = function(_, opts)
      ...
      dap.configurations.go = {
        -- debug main.go
        { name = "Launch Go main.go", type = "go_launch", request = "launch", program = "${workspaceFolder}/main.go", },
        -- To attach to a `dlv debug`
        { name = "Connect Go (remote dlv)", type = "go", request = "attach", mode = "remote", },
      ...
      if vim.fn.filereadable(project_dap) == 1 then
        ...
        if ok then
          if projectConfigs.adapters then
            vim.cmd('echomsg "DAP: loading project adapters..."')
            for name, adapter in pairs(projectConfigs.adapters) do
              dap.adapters[name] = adapter
              vim.cmd('echomsg "  - ' .. name .. '"')
            end
          end
          if projectConfigs.configurations then
            vim.cmd('echomsg "DAP: loading project configurations..."')
  ...
  },
  {
    ...
    keys = {
      ...
      { "<S-F9>", function() require("dap").set_breakpoint(vim.fn.input("Breakpoint condition: ")) end, desc = "DAP Conditional Breakpoint" },
